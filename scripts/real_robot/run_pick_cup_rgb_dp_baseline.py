#!/usr/bin/env python3
"""Build, validate, configure, and train the real-robot pick-cup RGB-DP baseline."""
...
def dataset_fingerprint(path: Path) -> dict[str, Any]:
    """Fingerprint dataset identity without hashing the large image payload."""
...
def make_config(
    ...
    *,
    ...
    if min(
        ...
        raise ValueError("epochs, steps, and batch size must be positive")
    ...
    experiment_name = f"pick_cup_rgb_dp_{options.sampler}_s{options.seed}"
    if options.smoke:
...
def write_config(config: dict[str, Any], config_dir: Path) -> Path:
    ...
    print(f"[pick_cup RGB-DP] wrote config: {path}", flush=True)
    return path
...
def standard_loader_preflight(config_dict: dict[str, Any]) -> dict[str, Any]:
    """Exercise the unmodified robomimic loader on both train and valid masks."""
    ...
    try:
        ...
        for label, sample in (("train", train_sample), ("valid", valid_sample)):
            ...
            if actions.shape != (17, 7) or not np.all(np.isfinite(actions)):
                raise ValueError(
                    f"{label} loader action sample has invalid shape/content: {actions.shape}"
                )
            for key in RGB_KEYS:
...
def canonical_training_config(payload: dict[str, Any]) -> dict[str, Any]:
    """Materialize defaults and remove only fields injected after data loading."""
...
def _resume_run_dir(config: dict[str, Any]) -> Path:
    """Resolve and validate the exact run directory robomimic will resume."""
    ...
    if flat_models.is_dir() and any(
        ...
        run_dirs = sorted(
            child for child in root.iterdir() if child.is_dir() and (child / "models").is_dir()
        )
        if not run_dirs:
...
def process_env(*, smoke: bool) -> dict[str, str]:
    ...
    env["PYTHONPYCACHEPREFIX"] = "/tmp/robomimic_pick_cup_rgb_dp_pycache"
...
def train(config_path: Path, config: dict[str, Any], *, resume: bool) -> Path:
    ...
    if existing_checkpoint is not None:
        print(
            f"[pick_cup RGB-DP] using existing completed checkpoint: "
    ...
    if resume:
        ...
        print(f"[pick_cup RGB-DP] resuming compatible run: {resume_dir}", flush=True)
    ...
    print(f"[pick_cup RGB-DP] training target: {checkpoint}", flush=True)
    return checkpoint
