#!/usr/bin/env python3
"""Evaluate a trained round-1 5 Hz pick-cup Diffusion Policy checkpoint.

This evaluator deliberately reports two different quantities:

1. the native validation objective, evaluated with the online network; and
2. open-loop action replay, evaluated with the EMA network used for rollout.

The validation split is traversed sequentially with ``drop_last=False`` so that
every held-out window is evaluated exactly once per requested random seed.
"""
...
from scripts.real_robot.run_pick_cup_rgb_dp_5hz_round1 import (  # noqa: E402
    dataset_fingerprint,
)


EVALUATOR_VERSION = "pick_cup_rgb_dp_round1_5hz_eval_v1"
ACTION_DIM = 7
MOTION_DIM = 6


class ActionReplayAccumulator:
    """Accumulate action errors without retaining predictions in memory."""
    ...
    def update(self, prediction: np.ndarray, target: np.ndarray) -> None:
        ...
        target_gripper = flat_target[:, 6]
        if not np.all(np.isin(target_gripper, (-1.0, 1.0))):
            raise ValueError("held-out gripper targets must be dense -1 or +1")

        error = flat_prediction[:, :MOTION_DIM] - flat_target[:, :MOTION_DIM]
        absolute_error = np.abs(error)
        self.action_count += int(flat_prediction.shape[0])
        self.motion_abs_sum += float(absolute_error.sum())
        self.motion_sq_sum += float(np.square(error).sum())
        self.motion_abs_by_dim += absolute_error.sum(axis=0)
        self.target_motion_abs_sum += float(
            np.abs(flat_target[:, :MOTION_DIM]).sum()
        )
        self.translation_sq_norm_sum += float(
            np.square(error[:, :3] * self.translation_scale_m).sum()
        )
        self.rotation_sq_norm_sum += float(
            np.square(error[:, 3:6] * self.rotation_scale_rad).sum()
        )

        predicted_gripper = np.where(flat_prediction[:, 6] >= 0.0, 1.0, -1.0)
        self.gripper_correct += int(
            np.count_nonzero(predicted_gripper == target_gripper)
        )
        ...
        self.prediction_min = min(
            self.prediction_min,
            float(flat_prediction.min()),
        )
        self.prediction_max = max(
            self.prediction_max,
            float(flat_prediction.max()),
        )
    ...
    def result(self) -> dict[str, Any]:
        ...
        return {
            ...
            "zero_motion_baseline_mae": (
                self.target_motion_abs_sum / motion_values
            ),
...
def _dataset_contract(
    ...
    if len(data) != 1:
        raise ValueError("the 5 Hz round-1 evaluator requires exactly one shard")
    ...
    with h5py.File(path, "r") as dataset:
        if validation_key not in dataset["mask"]:
            raise KeyError(f"dataset has no validation mask {validation_key!r}")
        validation_demos = decode_hdf5_strings(dataset[f"mask/{validation_key}"][:])
    ...
    if not math.isclose(
        float(action_manifest["hz"]), CONTROL_HZ, rel_tol=0.0, abs_tol=1e-12
    ):
        raise ValueError("checkpoint dataset is not a 5 Hz action dataset")
    if not math.isclose(
        float(action_manifest["translation_scale_m"]),
        TRANSLATION_SCALE_M,
        rel_tol=0.0,
        abs_tol=1e-12,
    ):
        raise ValueError("translation scale does not match the 5 Hz contract")
    if not math.isclose(
        float(action_manifest["rotation_scale_rad"]),
        ROTATION_SCALE_RAD,
        rel_tol=0.0,
        abs_tol=1e-12,
    ):
        raise ValueError("rotation scale does not match the 5 Hz contract")
    return path, {
        ...
        "rotation_aggregation": action_manifest["rotation_aggregation"],
    }
...
def evaluate_online_validation_loss(
    *,
    ...
    for seed in seeds:
        ...
        print(
            f"[5 Hz eval] online loss seed={seed}: {value:.8f} "
            f"({windows} windows)",
            file=sys.stderr,
            flush=True,
        )
...
def evaluate_ema_action_replay(
    *,
    ...
    """Generate rollout actions with EMA weights and compare current targets."""
    ...
    if (observation_horizon, action_horizon, prediction_horizon) != (2, 1, 4):
        raise ValueError(
            "expected 5 Hz horizons To/Ta/Tp=2/1/4, got "
    ...
    for seed in seeds:
        ...
        print(
            f"[5 Hz eval] EMA replay seed={seed}: "
    ...
    return {
        "description": (
            "EMA rollout action at slot To-1 compared with the demonstrated "
            "current action; this is open-loop imitation, not task success"
        ),
...
def evaluate_checkpoint(
    ...
    *,
    ...
    print(
        f"[5 Hz eval] loading {checkpoint} on {device}",
        file=sys.stderr,
        flush=True,
    )
    ...
    if config_dict["train"]["action_config"] != {
        ...
        raise ValueError("the evaluator requires the unnormalized 5 Hz action contract")
    ...
    checkpoint_report = {
        ...
        "uses_ema_for_rollout": model.ema is not None,
    }

    # The loaded archive also contains optimizer states. They are not needed for
    # evaluation, so release the duplicate checkpoint dictionary before replay.
    del checkpoint_dict
    gc.collect()
    if device.type == "cuda":
...
def main(argv: Sequence[str] | None = None) -> dict[str, Any]:
    ...
    if args.output is not None:
        ...
        print(f"[5 Hz eval] wrote report: {output}", file=sys.stderr, flush=True)
