#!/usr/bin/env python3
"""Build a 20 Hz real-robot mixed dataset used by RGB IDQL.
...
DEFAULT_HUMAN_DATASETS = (
    ROOT
    / "datasets/real_robot/pick_cup/pick_cup_rgb.hdf5",
)
...
DEFAULT_HUMAN_DATASETS_HELP = (
    ...
    "(defaults to the fixed 45-episode train split)."
)
REQUIRED_OBS_SHAPES = {
...
def _normalized_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    defaults = {
        ...
        "actor_condition_mode": DEFAULT_ACTOR_CONDITION_MODE,
        "seed": 0,
        "overwrite": False,
        "validate_only": False,
    }
...
def _expert_critic_validity(key: str, count: int) -> np.ndarray:
    values = np.ones(count, dtype=np.uint8)
    if key == "chunk_critic_valid":
        # Human teleoperation has no policy-proposal boundary. Use the same
        # non-overlapping H8 decision cadence as rollout proposals so filtering
        # does not accidentally change the intended source mixture by ~8x.
        values.fill(0)
        values[::CRITIC_CHUNK_HORIZON] = 1
    return values


def _override_source_critic_validity(source_label: str, key: str) -> bool:
    """Whether the mixed dataset replaces a validity mask from its source.

    Episode-layout chunk IDQL uses every causally aligned human action row as
    an H8 window start, while retaining the proposal-boundary mask embedded in
    policy rollouts. One-step configurations preserve their source masks.
    """

    if EXPERT_CHUNK_VALIDITY_MODE not in EXPERT_CHUNK_VALIDITY_MODES:
        raise ValueError(
            "EXPERT_CHUNK_VALIDITY_MODE must be one of "
            f"{EXPERT_CHUNK_VALIDITY_MODES}, got "
            f"{EXPERT_CHUNK_VALIDITY_MODE!r}"
        )
    return (
        EXPERT_CHUNK_VALIDITY_MODE == "stride_one"
        and source_label == "expert"
        and key == "chunk_critic_valid"
    )


def _critic_validity_values(
    source_episode: h5py.Group,
    source_label: str,
    key: str,
    count: int,
) -> np.ndarray:
    """Resolve the exact validity vector written into a mixed episode."""

    if _override_source_critic_validity(source_label, key):
        return np.ones(count, dtype=np.uint8)
    if key in source_episode:
        values = np.asarray(source_episode[key][:], dtype=np.uint8).reshape(-1)
        if values.shape != (count,) or np.any(~np.isin(values, (0, 1))):
            raise ValueError(
                f"{source_episode.name}/{key} must be a binary vector of "
                f"length {count}"
            )
        return values
    if source_label == "expert":
        return _expert_critic_validity(key, count)
    raise ValueError(
        f"{source_episode.name} is missing {key}; rebuild the processed "
        "rollout with proposal-aligned temporal metadata"
    )
    ...
    with h5py.File(source_path, "r") as source:
        ...
        for key in source_episode.keys():
            if key in RESERVED_EPISODE_KEYS:
                continue
            if REQUIRE_CRITIC_VALIDITY and _override_source_critic_validity(
                source_label, key
            ):
                # The source remains immutable because it is also part of the
                # completed one-step dataset lineage. Materialize only this
                # mixed-dataset mask instead of linking the source mask.
                continue
        ...
        if REQUIRE_CRITIC_VALIDITY:
            for key in CRITIC_VALIDITY_KEYS:
                if key in source_episode and not _override_source_critic_validity(
                    source_label, key
                ):
                    continue
                target.create_dataset(
                    key,
                    data=_critic_validity_values(
                        source_episode,
                        source_label,
                        key,
                        count,
...
def validate_existing(raw_args: argparse.Namespace) -> dict[str, Any]:
    ...
    critic_validity_counts = {
        key: {label: 0 for label in SOURCE_LABELS}
        for key in CRITIC_VALIDITY_KEYS
    }
    ...
    with h5py.File(args.output, "r") as output:
        expected_attrs = {
            ...
            "expert_chunk_validity_mode": EXPERT_CHUNK_VALIDITY_MODE,
        }
        ...
        if "data" not in output or "mask" not in output:
            ...
            for index, (label, source_path, source_key) in enumerate(records):
                ...
                with h5py.File(source_path, "r") as source:
                    ...
                    if REQUIRE_CRITIC_VALIDITY:
                        for validity_key in CRITIC_VALIDITY_KEYS:
                            try:
                                expected_validity = _critic_validity_values(
                                    source_episode,
                                    label,
                                    validity_key,
                                    count,
                                ...
                                errors.append(
                                    f"{location} cannot resolve {validity_key}: {exc}"
                                )
                                continue
                            if (
                                validity_key not in episode
                                or not np.array_equal(
                                    episode[validity_key][:],
                                    expected_validity,
                                )
                            ):
                                errors.append(
                                    f"{location}/{validity_key} has incorrect values"
                                )
                            critic_validity_counts[validity_key][label] += int(
        ...
        "critic_validity": (
            {
                key: {
                    "human": counts["expert"],
                    "success_rollout": counts["non_expert_success"],
                    "failure_rollout": counts["non_expert_failure"],
                    "total": sum(counts.values()),
                }
                for key, counts in critic_validity_counts.items()
            }
            if REQUIRE_CRITIC_VALIDITY
            else None
        ),
...
def build(raw_args: argparse.Namespace) -> dict[str, Any]:
    ...
    with _atomic_output(args) as staged:
        with h5py.File(staged, "w", libver="latest") as output:
            ...
            output.attrs["expert_chunk_validity_mode"] = (
                EXPERT_CHUNK_VALIDITY_MODE
            )
            output.attrs["sampling_definition"] = (
                f"one concatenated {args.selection_role} dataset; "
                + (
                    (
                        "stride-one human H8 starts plus source proposal-valid "
                        "rollout starts are selected by the sparse IDQL loader"
                        if EXPERT_CHUNK_VALIDITY_MODE == "stride_one"
                        else "source proposal-valid indices are selected by the "
                        "sparse IDQL loader"
                    )
                    if REQUIRE_CRITIC_VALIDITY
                    else "uniform over SequenceDataset indices"
                )
            )
            output.attrs["critic_validity_keys"] = _json(
                list(CRITIC_VALIDITY_KEYS) if REQUIRE_CRITIC_VALIDITY else []
            )
            output.attrs["critic_chunk_horizon"] = (
                CRITIC_CHUNK_HORIZON if REQUIRE_CRITIC_VALIDITY else 0
            )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=DEFAULT_ACTOR_CONDITION_MODE,
    )
